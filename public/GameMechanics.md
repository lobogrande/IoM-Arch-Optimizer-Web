...
*   **Stat Caps:** Stats have strict maximum allocations. `Str` and `Agi` cap at 50. `Int`, `Per`, and `Luck` cap at 30. `Div` and `Corr` cap at 15. 
*   **Cap Expansion:** Upgrade 45 ("Exp Gain/All Stat Cap Inc.") is a single-level purchase that provides a global +5 increase to all stat caps (e.g., bumping Str/Agi to 55, Luck to 35, and Corr to 20). Unlike Upgrade 12, Upgrade 45 is **not** Ascension-locked; it is available in all Ascension states once Floor 42 is reached.
